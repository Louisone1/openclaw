...
describe("doctor runtime tool schema checks", () => {
  ...
  it("reports active bundle MCP tool schemas that would be quarantined before a model turn", async () => {
    mocks.createBundleMcpToolRuntime.mockResolvedValueOnce({
      tools: [
        ...
        bundleMcpTool("fuzzplugin__fuzz_move_delta", {
          type: "array",
          items: { type: "number" },
        }),
      ],
    ...
    await expect(
      ...
      severity: "error",
      message:
        "Agent main tool fuzzplugin__fuzz_move_delta from plugin bundle-mcp has an unsupported input schema for runtime projection.",
      path: "mcp.servers",
      target: "fuzzplugin__fuzz_move_delta",
      requirement: 'fuzzplugin__fuzz_move_delta.parameters.type must be "object"',
      fixHint:
  ...
  it("reports unsupported schemas exposed only to a non-default configured agent", async () => {
    mocks.createOpenClawCodingTools.mockImplementation((options) =>
      options?.agentId === "worker"
        ? [tool("fuzz_move_delta", { type: "array", items: { type: "number" } })]
    ...
    await expect(
      ...
      severity: "error",
      message:
        "Agent worker tool fuzz_move_delta has an unsupported input schema for runtime projection.",
      path: "tools.fuzz_move_delta",
      target: "fuzz_move_delta",
      requirement: 'fuzz_move_delta.parameters.type must be "object"',
      fixHint:
  ...
  it("reports unreadable active tool rows without failing while building doctor findings", async () => {
    const tools: AnyAgentTool[] = new Proxy(
      [tool("fuzz_move_delta", { type: "object", properties: {} })],
      {
  ...
  it("loads bundled MCP runtime once per distinct agent workspace", async () => {
    ...
    await expect(
      ...
      severity: "error",
      message:
        "Agent worker tool fuzzplugin__fuzz_move_delta from plugin bundle-mcp has an unsupported input schema for runtime projection.",
      path: "mcp.servers",
      target: "fuzzplugin__fuzz_move_delta",
      requirement: 'fuzzplugin__fuzz_move_delta.parameters.type must be "object"',
      fixHint:
